...
import os 
import time
import shutil # Necessário para o modo Windows
import traceback
...
# --- LEITURA INTELIGENTE (DETECTA O SISTEMA OPERACIONAL) ---
def safe_read_db(db_path, query):
    """
    Adapta a estratégia de leitura baseada no Sistema Operacional.
    """
    # MODO WINDOWS (DEV): Usa Shadow Copy para evitar File Lock
    if os.name == 'nt':
    ...
    # MODO LINUX (PROD): Usa Leitura Direta (Mais rápido e robusto no Linux)
    else:
        for _ in range(5):
            try:
...
app.layout = dbc.Container([
    ...
    html.Div([
        dbc.Row([
            ...
            dbc.Col([
                html.Div(create_card("B3 - Dólar Futuro (BRL)", "grid-dolar"), className="pb-1", style={"flex": "2"}),
                html.Div(dbc.Card([
                    dbc.CardHeader([
                        html.Span("ANÁLISE GRÁFICA (90D)", className="fw-bold text-white", style={"fontSize": "0.85rem"}),
                    ...
                    dbc.CardBody(dcc.Graph(id="chart-main", style={"height": "100%"}, config={'displayModeBar': False}), className="p-1 h-100")
...
def update_all(n, n_open, n_apply, is_open, sel_exchange, sel_prod, sel_symbol, current_filter_data):
    ...
    if trigger == 'open-filter': return [no_update]*11 + [True, "", no_update]
    ...
    if trigger == 'apply-filter':
        ...
        save_active_chart(sel_symbol, "57")
        is_open = False
    ...
    try:
        ...
        if df.empty: return [no_update]*14 # Mantém estável
