...
# Arquivos Sombra (Opcional, mas mantido para robustez)
DB_REALTIME_SHADOW = os.path.join(BASE_DIR, 'shadow_realtime.duckdb')
DB_HISTORY_SHADOW = os.path.join(BASE_DIR, 'shadow_history.duckdb')

CONFIG_FILE = os.path.join(BASE_DIR, 'config.json')
CHART_FILE = os.path.join(BASE_DIR, 'active_chart.json')

# --- UTILITÁRIOS ---
def get_shadow_db(original_path, shadow_path):
    try:
...
def safe_read_db(original_path, shadow_path, query):
    """Lê o banco. Se falhar, retorna DataFrame VAZIO para tratar depois."""
    target_db = get_shadow_db(original_path, shadow_path)
    if not target_db: return pd.DataFrame()
    try:
...
def create_card(title, grid_id):
    return dbc.Card([
        ...
        dbc.CardBody(dag.AgGrid(
            ...
            getRowId="params.data.symbol", # EVITA PISCAR SCROLL
            dashGridOptions={
...
app.layout = dbc.Container([
    ...
    html.Div([
        dbc.Row([
            dbc.Col([html.Div(create_card("CBOT - Soja (USD/bu)", "grid-soja"), className="pb-1", style={"flex": "1"}), html.Div(create_card("CBOT - Farelo (USD/st)", "grid-farelo"), className="pb-1", style={"flex": "1"}), html.Div(create_card("CBOT - Óleo (USD/lb)", "grid-oleo"), style={"flex": "1"})], width=4, className="d-flex flex-column h-100 pe-1"),
            dbc.Col([html.Div(create_card("CBOT - Milho (USD/bu)", "grid-milho"), className="pb-1", style={"flex": "1"}), html.Div(create_card("B3 - Milho Futuro (BRL)", "grid-ccm"), className="pb-1", style={"flex": "1"}), html.Div(create_card("B3 - Boi Gordo (BRL)", "grid-boi"), style={"flex": "1"})], width=4, className="d-flex flex-column h-100 px-1"),
            dbc.Col([html.Div(create_card("B3 - Dólar Futuro (BRL)", "grid-dolar"), className="pb-1", style={"flex": "2"}), html.Div(dbc.Card([dbc.CardHeader([html.Span("ANÁLISE GRÁFICA (90D)", className="fw-bold text-white", style={"fontSize": "0.85rem"}), dbc.Button(html.I(className="fa-solid fa-filter"), id="open-filter", color="light", size="sm", className="bg-transparent border-0 text-white")], className="d-flex justify-content-between align-items-center bg-black border-bottom border-secondary py-1 ps-2 pe-2"), dbc.CardBody(dcc.Graph(id="chart-main", style={"height": "100%"}), className="p-1 h-100")], className="h-100 border-secondary shadow-sm"), style={"flex": "1"})], width=4, className="d-flex flex-column h-100 ps-1"),
...
def update_all(n, n_open, n_apply, is_open, sel_exchange, sel_prod, sel_symbol, current_filter_data):
    ...
    # LISTA DE ESTABILIDADE: Inicializa tudo como NO_UPDATE para não piscar a tela
    res_grids = [no_update] * 7
    res_chart = no_update
    res_time = no_update
    res_opts = [no_update] * 2
    res_modal = no_update
    res_msg = no_update
    res_store = no_update
    ...
    if trigger == 'open-filter': 
        return res_grids + [res_chart, res_time] + res_opts + [True, "", no_update]
    
    # 2. APLICAR FILTRO
    if trigger == 'apply-filter':
        if not all([sel_exchange, sel_prod, sel_symbol]): 
            return res_grids + [res_chart, res_time] + res_opts + [True, "Preencha todos!", no_update]
        ...
        # Fecha modal e salva filtro, mas não atualiza dados (espera próximo tick)
        return res_grids + [res_chart, res_time] + res_opts + [False, "", current_filter_data]

    try:
        # 3. LEITURA DE DADOS
        df = safe_read_db(DB_REALTIME, DB_REALTIME_SHADOW, "SELECT * FROM market_snapshot ORDER BY Maturity ASC, symbol ASC")
        df_hist = safe_read_db(DB_HISTORY, DB_HISTORY_SHADOW, "SELECT * FROM market_history ORDER BY date_ref ASC")
        
        # --- ESTABILIDADE DE DADOS VAZIOS ---
        # Se a leitura falhar (df vazio), NÃO retornamos [], retornamos no_update.
        # Isso impede que a tabela fique branca se o ingestor travar o arquivo por 1 segundo.
        if df.empty:
            # print("⚠️ Skip update (Dados vazios)")
            return [no_update]*14

        # Se chegou aqui, TEM DADOS. Processa e atualiza.
        ...
        # Monta as listas
        rows_soja = d('CBOT', 'Soja')
        rows_milho_cbot = d('CBOT', 'Milho')
        rows_milho_b3 = d('B3', 'Milho') # Separado por bolsa
        rows_farelo = d('CBOT', 'Farelo')
        rows_oleo = d('CBOT', 'Oleo')
        rows_dolar = d('B3', 'Dolar')
        rows_boi = d('B3', 'Boi')
        ...
        if not target_symbol: # Auto-Select
            dol = df[df['product_name'].str.lower() == 'dolar']
            if not dol.empty: 
                target_symbol = dol.iloc[0]['symbol']
        ...
        return (rows_soja, rows_milho_cbot, rows_farelo, rows_oleo, # Atenção à ordem!
                rows_dolar, rows_milho_b3, rows_boi,
                fig, datetime.now().strftime('%H:%M:%S'), prods, symbols, is_open, "", current_filter_data)

    except Exception as e:
        print(f"❌ Erro App: {e}")
        # Em caso de erro, MANTÉM O QUE ESTAVA (Não pisca a tela)
        return [no_update]*14
